...
"""
read file considering num1 rpeaks (each about num2 points, and then cast it to dim1 window, each window contains dim2 points
"""
...
#sequenced features
def load_sample(path, name, real_label, sampling_rate, num1, num2, train=True,dimension1=4,dimension2=500):
    ...
    if (real_label == 'Normal') | (train is False):
        for index in range(0, len(rpeaks)-dimension1, 1):
    ...
    sample_x = np.reshape(sample_x, [sample_x.shape[0], num1, num2])
