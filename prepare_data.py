...
#sequenced features
def load_sample(path, name, real_label, sampling_rate, num_beats,num_features, train=True):
    x_signal, y_signal = read_sample(path, name)
    length = y_signal.shape[0]
    rpeaks = detect_r_points(y_signal, length, sampling_rate)

    sample_x = np.empty([0, 1, num_features])
    ...
    base_features = load_base_features(x_signal, y_signal, sampling_rate, real_label, rpeaks)
    if (real_label == 'Normal') | (train is False):
        #x = np.empty([1, num_windows])
        for index in range(0, len(rpeaks)-num_beats, num_beats):
            window_features = load_window_features(x_signal, y_signal, sampling_rate, real_label, rpeaks, index, num_beats)
            x = np.append(np.array(base_features), np.array(window_features))
            x = np.reshape(x, [1,x.shape[0]])
            #print('Num features = %d  %d' %(x.shape[0], x.shape[1]))
            y = np.array(label)
        ...
        for p in problems:
            problem = int(p)
            for index in range(problem, problem + num_beats):
                if (index > 0) & (index + num_beats < rpeaks.shape[0]):
                    window_features = load_window_features(x_signal, y_signal, sampling_rate, real_label, rpeaks, index, num_beats)
                    x = np.append(np.array(base_features), np.array(window_features))
                    x = np.reshape(x, [1,x.shape[0]])
                    y = np.array(label)
